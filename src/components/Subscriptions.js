...
const Subscriptions = () => {
  ...
  const renderCancelModal = () => {
    if (cancelStep === 'confirm') {
      return (
        <>
          <div style={{ fontWeight: 600, fontSize: 18, textAlign: 'center', marginBottom: 24 }}>Are You Sure You Want To Cancel This Subscription?</div>
    ...
    if (cancelStep === 'number') {
      return (
        <>
          <div style={{ fontWeight: 600, fontSize: 18, textAlign: 'center', marginBottom: 12 }}>Enter Number To Cancel Subscriptions</div>
          <div style={{ fontSize: 14, textAlign: 'center', marginBottom: 16 }}>Enter Your New Phone Number to change</div>
          <Input
            ...
            maxLength={8}
          />
          <Button type="primary" style={{ width: '100%' }} onClick={() => setCancelStep('otp')}>Continue</Button>
        </>
      );
    }
    if (cancelStep === 'otp') {
      return (
        <>
          <div style={{ fontWeight: 600, fontSize: 18, textAlign: 'center', marginBottom: 12 }}>Enter OTP Sent To Cancel Subscriptions</div>
          <div style={{ fontSize: 14, textAlign: 'center', marginBottom: 16 }}>Enter Your New Phone Number to change</div>
          ...
          <div style={{ textAlign: 'center', marginBottom: 16, color: '#039be5' }}>Resend in {otpTimer}s</div>
          <Button type="primary" style={{ width: '100%' }} onClick={() => setCancelStep('done')}>Continue</Button>
        </>
      );
    }
    if (cancelStep === 'done') {
      return (
        <>
          <div style={{ fontWeight: 600, fontSize: 20, textAlign: 'center', marginBottom: 24 }}>Subscription Cancelled</div>
          <Button type="primary" style={{ width: '100%' }} onClick={() => { setCancelStep(null); setSelectedPlan(null); }}>Done</Button>
        </>
      );
    }
    return null;
      ...
      {!selectedPlan ? (
        <>
          ...
          <div className="subscriptions-desc">
            Lorem ipsum dolor sit amet consectetur. Leo vitae tellus turpis adipiscing in. Eget in vehicula et egestas risus sit lacus. Sit et ut ac vulputate. Scelerisque euismod phasellus dignissim ut.
          </div>
          <Tabs
            activeKey={activeTab}
            onChange={key => { setActiveTab(key); setSelectedPlan(null); }}
            items={[{ key: 'Individual', label: 'Individual' }, { key: 'Dealer', label: 'Dealer' }]}
            className="subscriptions-tabs"
          />
          <div className="subscriptions-list" style={{ display: 'flex', gap: 24, marginTop: 24 }}>
            {plans.length === 0 ? <EmptyState /> : plans.map(plan => (
              <div key={plan.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedPlan(plan)}>
