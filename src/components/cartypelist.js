...
const CarTypeList = ({ setSearchBodyType }) => {
  const [loading, setLoading] = useState(false);
  const [carBodyTypes, setCarBodyTypes] = useState([]); // ✅ store API body types
  const [carSearch, setCarSearch] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [bodyType, setBodyType] = useState("All Body Types");
  const navigate = useNavigate();
        ...
        {carBodyTypes.map((type) => (
          ...
          >
            <div className="car-type-icon">
              <img
                src={`http://13.202.75.187:5002${type.body_type_image}`} // ✅ prepend base URL
                alt={type.body_type}
                width="96"
                height="36"
              />
