  ...
  return (
    <div className="user-saved-searches-outer-card">
      {savedSearches.map((item, idx) => (
        <div className={`user-saved-search-section${idx < savedSearches.length - 1 ? ' with-divider' : ''}`} key={idx}>
          <div className="user-saved-search-header">
            <img src={item.logo} alt={item.brand} className="user-saved-search-logo" />
            <div className="user-saved-search-title">
              <span className="user-saved-search-brand">{item.brand}</span>
              {item.model && <span className="user-saved-search-model">{item.model}</span>}
            </div>
          </div>
